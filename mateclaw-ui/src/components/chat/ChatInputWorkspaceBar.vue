  ...
  >
    ...
    <div v-if="pendingApproval?.status === 'pending_approval'" class="approval-bar">
      <div class="approval-bar__info">
        ...
        <!-- MetaY custom: 风险等级徽标（来自 pendingApproval.maxSeverity） -->
        <span v-if="approvalRisk" class="approval-bar__risk" :class="`is-${approvalRisk}`">
          {{ approvalRiskLabel }}
        </span>
      </div>
    ...
    <!-- MetaY custom: v1 composer 背景层（任意菜单打开时铺满 ChatInput 底栏） -->
    <div
      v-if="anyComposerMenuOpen"
      class="composer-menu-backdrop"
      @click="closeAllComposerMenus"
    ></div>
    <!-- MetaY custom: v1 风格 composer 底栏（+ 菜单 / 权限 pill / 工作区 chip / 模型 chip） -->
    <div v-if="workspaceName" class="composer-footer-left">
      <div class="composer-menu-wrap" @click.stop>
        <button
          class="composer-icon-btn"
          :class="{ active: addMenuOpen }"
          type="button"
          :title="t('chat.composer.addMenu', '添加')"
          @click="onToggleAddMenu"
        >
          <span class="composer-icon-btn__plus">+</span>
        </button>
        <div v-if="addMenuOpen" class="composer-popover composer-popover--menu">
          ...
          <button
            ...
            type="button"
            @click="onTogglePlanMode"
          >
            <span class="composer-menu-item__icon">≋</span>
            <span>{{ t('chat.composer.planMode', 'Plan mode') }}</span>
            <span class="composer-switch" :class="{ 'is-on': planMode }">
              <span class="composer-switch__thumb"></span>
            </span>
          </button>
          <button class="composer-menu-item" type="button" @click="onOpenPlugins">
            <span class="composer-menu-item__icon">⌘</span>
            <span>{{ t('chat.composer.plugins', 'Plugins') }}</span>
      ...
      <div class="composer-menu-wrap" @click.stop>
        <button
          class="composer-pill-btn is-full-access"
          type="button"
          :title="t('chat.composer.fullAccess', '完全访问')"
          @click="onTogglePermissionMenu"
        >
          <span>{{ t('chat.composer.fullAccess', '完全访问') }}</span>
          <span class="composer-btn-caret">▾</span>
        </button>
        <div v-if="permissionMenuOpen" class="composer-popover composer-popover--list">
          <button
            class="composer-option"
            type="button"
            :class="{ active: permissionLevel === 'limited' }"
            @click="onPickPermissionLevel('limited')"
          >
            <span class="composer-option__content">
              <span>{{ t('chat.composer.limitedAccess', '默认权限') }}</span>
              <span class="composer-option__desc">{{ t('chat.composer.limitedAccessDesc', '工作区内常规读写默认允许；敏感路径和危险命令仍可能触发审批或拦截。') }}</span>
            </span>
            <span v-if="permissionLevel === 'limited'" class="composer-option__check">✓</span>
          </button>
          <button
            class="composer-option composer-option--danger"
            type="button"
            :class="{ active: permissionLevel === 'full' }"
            @click="onPickPermissionLevel('full')"
          >
            <span class="composer-option__content">
              <span>{{ t('chat.composer.fullAccess', '完全访问') }}</span>
              <span class="composer-option__desc">{{ t('chat.composer.fullAccessDesc', '自动放行原本需要审批的受控操作；仍受工作区边界和硬阻断规则限制。') }}</span>
            </span>
            <span v-if="permissionLevel === 'full'" class="composer-option__check">✓</span>
          </button>
        </div>
      </div>
      <div class="composer-menu-wrap" @click.stop>
        <button
          ...
          type="button"
          :title="t('chat.composer.workspace', '工作区')"
          @click="onWorkspaceChipClick"
        >
          <span class="composer-workspace-chip__icon">🗂️</span>
          <span class="composer-workspace-chip__label">{{ workspaceName }}</span>
          <span v-if="workspaceBasePath" class="composer-workspace-chip__path">· {{ workspaceBasePath }}</span>
          <span class="composer-btn-caret">▾</span>
        </button>
        <div v-if="workspaceMenuOpen" class="composer-popover composer-popover--workspace">
          <!-- === MetaY custom: v1 风格双段工作区弹层 === -->
          <section class="composer-popover__section">
            <div class="composer-popover__label">{{ t('chat.composer.workspacePopoverTitle', '切换 WORKSPACE') }}</div>
            <p class="composer-popover__hint">{{ t('chat.composer.workspaceDesktopHint', '仅桌面客户端可直接选择本地文件夹，浏览器环境不允许安全获取可绑定的本地绝对路径。') }}</p>
            <button class="composer-menu-item" type="button" @click="onWorkspaceAction('permission')">
              <span class="composer-menu-item__icon">⚙</span>
              <span class="composer-menu-item__main">
                <span class="composer-menu-item__label">{{ t('chat.composer.workspaceSettings', '工作空间设置') }}</span>
              </span>
              <span class="composer-menu-item__caret">›</span>
            </button>
          </section>
          <section v-if="switchableWorkspaces.length" class="composer-popover__section">
            <div class="composer-popover__label">{{ t('chat.composer.workspaceSection', '工作空间') }}</div>
            <button
              v-for="ws in switchableWorkspaces"
              :key="ws.id"
              class="composer-workspace-item"
              type="button"
              :class="{ 'is-active': String(ws.id) === String(activeWorkspaceId) }"
              @click="onPickWorkspace(ws.id)"
            >
              <span class="composer-workspace-item__name">{{ ws.name }}</span>
              <span v-if="String(ws.id) === String(activeWorkspaceId)" class="composer-workspace-item__check">✓</span>
            </button>
          </section>
        </div>
      </div>
      <span v-if="modelLabel" class="composer-model-inline">{{ modelLabel }}</span>
    </div>
 ...
 */
// === MetaY custom start === 审批风险等级徽标
const approvalRisk = computed<string | null>(() => {
  const sev = (props.pendingApproval as any)?.maxSeverity
  return sev ? String(sev).toLowerCase() : null
})
const approvalRiskLabel = computed(() => {
  const map: Record<string, string> = {
    low: t('teacher.risk.low', '低风险'),
    medium: t('teacher.risk.medium', '中风险'),
    high: t('teacher.risk.high', '高风险'),
    critical: t('teacher.risk.critical', '严重'),
  }
  return approvalRisk.value ? (map[approvalRisk.value] || approvalRisk.value) : ''
})
// === MetaY custom end ===
...
// === MetaY custom start === v1 风格 composer 底栏 popover 逻辑
const addMenuOpen = ref(false)
const permissionMenuOpen = ref(false)
const workspaceMenuOpen = ref(false)
const planMode = ref(false)
const permissionLevel = ref<'full' | 'limited'>('full')
const workspaceStore = useWorkspaceStore()
const switchableWorkspaces = computed(() => workspaceStore.workspaces || [])
const activeWorkspaceId = computed(() => workspaceStore.currentWorkspaceId)
...
const anyComposerMenuOpen = computed(() =>
  addMenuOpen.value || permissionMenuOpen.value || workspaceMenuOpen.value
)
...
function onTogglePlanMode() {
  ...
  emit('toggle-plan-mode', planMode.value)
}
...
function onTogglePermissionMenu() {
  const wasOpen = permissionMenuOpen.value
  closeAllComposerMenus()
  permissionMenuOpen.value = !wasOpen
}

function onPickPermissionLevel(level: 'full' | 'limited') {
  permissionLevel.value = level
  permissionMenuOpen.value = false
  emit('switch-permission-level', level)
}

function onWorkspaceChipClick() {
  const wasOpen = workspaceMenuOpen.value
  closeAllComposerMenus()
  workspaceMenuOpen.value = !wasOpen
  if (workspaceMenuOpen.value && switchableWorkspaces.value.length === 0) {
    workspaceStore.fetchWorkspaces()
  }
}

function onPickWorkspace(workspaceId: string | number) {
  workspaceMenuOpen.value = false
  if (String(workspaceId) === String(activeWorkspaceId.value)) return
  workspaceStore.switchWorkspace(String(workspaceId))
  // @ts-ignore TS 联合类型推断冲突，运行态正确
  emit('switch-workspace', workspaceId)
}

function onWorkspaceAction(action: 'switch' | 'permission') {
  workspaceMenuOpen.value = false
  if (action === 'permission') emit('open-permission')
  else emit('switch-workspace')
}

function onDocumentClick(e: MouseEvent) {
  const target = e.target as HTMLElement | null
  if (!target) return
  if (target.closest('.composer-menu-wrap')) return
  closeAllComposerMenus()
}
...
.talk-btn:hover:not(:disabled) {
  color: var(--mc-primary);
  background: var(--mc-primary-light, color-mix(in srgb, var(--mc-primary) 8%, transparent));
}
...
.send-btn:hover:not(:disabled) {
  background: var(--mc-primary-hover, #2f72de);
}
...
/* === MetaY custom === v1 风格 composer 底栏 */
...
.composer-workspace-chip:hover {
  border-color: color-mix(in srgb, var(--mc-primary) 28%, transparent);
  background: color-mix(in srgb, var(--mc-primary) 10%, var(--mc-bg-elevated, #f8fafc));
}
.composer-pill-btn {
  padding: 0 10px;
}
...
/* === MetaY custom === 行内模型显示（置于底栏末尾，无边框纯文字） */
/* === MetaY custom === v1 composer 背景层 */
...
/* === MetaY custom === v1 composer popover 弹层 */
.composer-menu-wrap {
  position: relative;
  display: inline-flex;
  align-items: center;
...
.composer-pill-btn.is-full-access .composer-btn-caret,
.composer-workspace-chip .composer-btn-caret {
  color: inherit;
}
.composer-popover {
  position: absolute;
...
.composer-workspace-item.is-active {
...
.composer-workspace-item__check {
  font-size: 12px;
  color: var(--mc-primary);
  font-weight: 700;
  margin-left: 8px;
}
.composer-popover__title {
  font-size: 11px;
  font-weight: 600;
  color: var(--mc-text-tertiary, #94a3b8);
...
.composer-workspace-summary {
  padding: 6px 10px 8px;
  border-bottom: 1px solid var(--mc-border, #e2e8f0);
  margin-bottom: 4px;
}
.composer-workspace-summary__name {
  font-size: 13px;
  font-weight: 600;
  color: var(--mc-text-primary, #1e293b);
}
.composer-workspace-summary__path {
  font-size: 11px;
  color: var(--mc-text-tertiary, #94a3b8);
  margin-top: 2px;
...
.composer-menu-item__desc {
  font-size: 11px;
  color: var(--mc-text-tertiary, #94a3b8);
  margin-top: 1px;
}
.composer-menu-item__check {
  font-size: 12px;
  color: var(--mc-primary);
  font-weight: 700;
}
/* === MetaY custom === v1 composer-switch 开关 */
...
/* === MetaY custom === v1 composer-option（权限 popover 选项） */
...
.composer-option__content > span:first-child {
  font-weight: 600;
  color: inherit;
}
...
.approval-bar__detail {
  ...
  padding: 6px 10px;
  border-radius: 8px;
  background: var(--mc-code-bg, color-mix(in srgb, var(--mc-primary) 8%, transparent));
  color: var(--mc-text-primary, #1e293b);
