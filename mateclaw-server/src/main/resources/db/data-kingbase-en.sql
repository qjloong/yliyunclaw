...
-- ==================== Built-in custom Agents (dev1 custom, MetaY) ====================
...
VALUES (1000000010, 'Teacher Exam Assistant', 'Junior Chinese classics reading exam designer with knowledge-base grounding.', 'plan_execute', 'You are a Teacher Exam Assistant, an exam-design expert for junior Chinese classics reading. Every fact must come from the bound knowledge base or session materials; never invent. Plan first, then draft items, default to providing answer keys and grading rubrics; self-audit for source, difficulty, coverage, and operability before output.', NULL, 8, TRUE, '📚', 'education,exam,teacher,builtin', 'builtin.teacher_exam_assistant', 'v1', 'education', 'junior_chinese_exam', 'teacher_exam_assistant_profile', 'capability.education.junior_chinese_exam', 'builtin.teacher_exam', NULL, 'Generate sourced, answered, rubric-scored middle-school classics reading exam items based on knowledge bases and session materials.', '[{"title":"Classic novel practice set","prompt":"Design a junior high classics reading practice set for «Journey to the West», with answers and rubric."},{"title":"Modern prose reading item","prompt":"Write a junior high modern prose reading item, with reference answer and grading criteria."},{"title":"Classical Chinese punctuation & translation","prompt":"Compose a classical Chinese punctuation and translation item, with rubric."},{"title":"Ancient poetry appreciation","prompt":"Write an ancient poetry appreciation item, with answer and grading reference."}]', NOW(), NOW(), 0)
ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, description=EXCLUDED.description, agent_type=EXCLUDED.agent_type, system_prompt=EXCLUDED.system_prompt, model_name=EXCLUDED.model_name, max_iterations=EXCLUDED.max_iterations, enabled=EXCLUDED.enabled, icon=EXCLUDED.icon, tags=EXCLUDED.tags, template_id=EXCLUDED.template_id, template_version=EXCLUDED.template_version, template_category=EXCLUDED.template_category, template_domain=EXCLUDED.template_domain, profile_id=EXCLUDED.profile_id, capability_pack_id=EXCLUDED.capability_pack_id, plugin_key=EXCLUDED.plugin_key, knowledge_base_ids_json=EXCLUDED.knowledge_base_ids_json, home_subtitle=EXCLUDED.home_subtitle, home_quick_starts_json=EXCLUDED.home_quick_starts_json, update_time=EXCLUDED.update_time, deleted=EXCLUDED.deleted;

INSERT INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, template_id, template_version, template_category, template_domain, profile_id, capability_pack_id, plugin_key, knowledge_base_ids_json, home_subtitle, home_quick_starts_json, create_time, update_time, deleted)
VALUES (1000000011, 'Coding Engineer Assistant', 'Codex-style coding agent for real repositories: understand, plan, edit, test, and review.', 'plan_execute', 'You are a Coding Engineer Assistant, a Codex-style engineering agent for real code repositories. Read context first, then propose the minimum necessary change, and finally prove it with tests, diffs, and a clear summary; risky shell commands, dependency installs, network access, deletions, git push, and PR creation must be approved per policy; never read, output, or share secrets or production configs.', NULL, 12, TRUE, '💻', 'coding,developer,builtin', 'builtin.coding_agent', 'v1', 'developer', 'codex_workspace', 'coding_agent_profile', 'capability.developer.codex_workspace', 'builtin.coding_agent', NULL, 'Coding assistant for real code repositories: understand, plan, modify, test, and review.', '[{"title":"Explain this code","prompt":"Please explain the logic and role of main in the current workspace."},{"title":"Draft a change plan","prompt":"I want to add a new REST endpoint to the project, please draft a change plan first."},{"title":"Fix a bug","prompt":"Tests failed; please locate and fix the related bug."},{"title":"Review my changes","prompt":"Please review my uncommitted changes and call out risks and improvements."}]', NOW(), NOW(), 0)
ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, description=EXCLUDED.description, agent_type=EXCLUDED.agent_type, system_prompt=EXCLUDED.system_prompt, model_name=EXCLUDED.model_name, max_iterations=EXCLUDED.max_iterations, enabled=EXCLUDED.enabled, icon=EXCLUDED.icon, tags=EXCLUDED.tags, template_id=EXCLUDED.template_id, template_version=EXCLUDED.template_version, template_category=EXCLUDED.template_category, template_domain=EXCLUDED.template_domain, profile_id=EXCLUDED.profile_id, capability_pack_id=EXCLUDED.capability_pack_id, plugin_key=EXCLUDED.plugin_key, knowledge_base_ids_json=EXCLUDED.knowledge_base_ids_json, home_subtitle=EXCLUDED.home_subtitle, home_quick_starts_json=EXCLUDED.home_quick_starts_json, update_time=EXCLUDED.update_time, deleted=EXCLUDED.deleted;
