...
public class OpenAIOAuthService {
     ...
     * Used by device-code completion where the verifier is returned inline
     * instead of being looked up from the local pending-state cache.
     */
    public void exchangeTokenWithVerifier(String code, String codeVerifier, String redirectUri) {
        if (!StringUtils.hasText(code)) {
            throw new MateClawException("err.llm.oauth_exchange_failed", "缺少 authorization code");
        }
        if (!StringUtils.hasText(codeVerifier)) {
            throw new MateClawException("err.llm.oauth_exchange_failed", "缺少 code_verifier");
        }

        String effectiveRedirectUri = StringUtils.hasText(redirectUri) ? redirectUri : REDIRECT_URI;

        String body = "grant_type=authorization_code"
                + "&client_id=" + enc(CLIENT_ID)
                + "&code=" + enc(code)
                + "&code_verifier=" + enc(codeVerifier)
                + "&redirect_uri=" + enc(effectiveRedirectUri);
