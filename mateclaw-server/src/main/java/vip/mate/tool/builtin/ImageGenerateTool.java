...
public class ImageGenerateTool {
    ...
    @vip.mate.tool.ConcurrencyUnsafe("creates async tasks and persists generated artifacts; provider rate limits also forbid parallel calls")
        @Tool(description = "Image generation tool. Supports actions: generate (default), list (show available providers), "
            + "status (check task status). IMPORTANT: when the user asks to create an image and a prompt is provided, "
            + "call generate directly instead of list. Some providers are async (30s-2min), results auto-displayed in conversation.")
    public String image_generate(
        ...
        String normalizedAction = resolveAction(action, prompt, size, aspectRatio, count, model, taskId);
    ...
    private String resolveAction(String action,
                                 ...
                                 String model,
                                 String taskId) {
        boolean hasGenerationIntent = (prompt != null && !prompt.isBlank())
                || (size != null && !size.isBlank())
                || (aspectRatio != null && !aspectRatio.isBlank())
                || count != null
                || (model != null && !model.isBlank());

        if (hasGenerationIntent) {
            if (action != null && !action.isBlank() && !"generate".equalsIgnoreCase(action.trim())) {
                log.info("[ImageGenerateTool] Override action '{}' to 'generate' because generation parameters are present",
                        action);
            }
            return "generate";
        }
        ...
        return switch (normalizedAction) {
            case "list", "status", "generate" -> normalizedAction;
            default -> "generate";
        };
    }
