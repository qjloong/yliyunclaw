 ...
 * MCP 连接是平台共享配置，不属于任何单一 Workspace。所有管理、测试和读取接口
 * 均要求 {@code mate_user.role=admin} 的平台管理员；Workspace owner/admin 只能在
 * 自己的 Workspace 中为 Agent 绑定已经发布的工具，不能查看或修改连接和凭据。
 *
